...
import {HomeAPIResponse} from "type/api";

export class HomeService {
    static getExploreAccountInfo(): Promise<HomeAPIResponse> {
    ...
    static getPlanAccountInfo(): Promise<HomeAPIResponse> {
        return new Promise((resolve, reject) => {
            const delay = Math.random() * 1000 + 200;
            setTimeout(resolve, delay, {
                code: 0,
    ...
    static getClaimsAccountInfo(): Promise<HomeAPIResponse> {
        return new Promise((resolve, reject) => {
            const delay = Math.random() * 1000 + 200;
            setTimeout(resolve, delay, {
                code: 0,
